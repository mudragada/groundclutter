...
so knowing cost per minute and cost per mile for each car type, as well as ride distance and ride time, return the fare estimates for all car types.
...
def fareEstimator(ride_time, ride_distance, cost_per_minute, cost_per_mile):
    fareEstimator = []
    lenCPM = len(cost_per_minute)
    for idx in range(0, lenCPM):
        fareEstimator.append(round(cost_per_minute[idx] * (ride_time) + cost_per_mile[idx] * ride_distance, 2))
